...
def add_book_via_api(owner_id, title, author, description, status, file=None):
    try:
        ...
        res = requests.post(f"{API_URL}/books/add", data=payload, files=files)
        if res.status_code == 200:
            return True, res.json().get("message", "Book added successfully.")
        else:
            return False, str(res.json().get("detail", "Unable to add the book."))
    except Exception as e:
        return False, str(e)
